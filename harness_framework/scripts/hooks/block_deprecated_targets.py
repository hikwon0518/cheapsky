#!/usr/bin/env python3
"""PreToolUse Write/Edit/MultiEdit 훅: Deprecated 소스 가드.

ADR-022 Deprecated (2026-04-18): Amadeus 포털 가입 중단. `services/amadeus.ts`
또는 유사 시세 클라이언트 생성을 Core 에서 차단. `CHEAPSKY_STAGE=stretch3` 에서만
해제 (stretch3 진입 + 신규 ADR 승인 후).
...
DEPRECATED_PATHS = [
    (re.compile(r"services[/\\](amadeus|kiwi|travelpayouts)", re.IGNORECASE),
     "ADR-022: Amadeus/Kiwi/Travelpayouts 시세 클라이언트"),
]
...
def main() -> int:
    ...
    for pat, label in DEPRECATED_PATHS:
        if pat.search(path):
            if stage == "stretch3":
                continue
            sys.stderr.write(
                ...
                "Stretch 3 진입 + 신규 ADR 승인 후 `export CHEAPSKY_STAGE=stretch3` 로 해제.\n"
            )
            return 2
